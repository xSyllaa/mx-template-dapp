-- Update leaderboard functions to work correctly with the fixed total_points

-- Drop old functions if they exist
DROP FUNCTION IF EXISTS get_leaderboard(TEXT, INTEGER, INTEGER, INTEGER, INTEGER, TEXT[]);
DROP FUNCTION IF EXISTS get_user_rank(UUID, TEXT, INTEGER, INTEGER, INTEGER, TEXT[]);
...
BEGIN
    ...
    WITH ranked_users AS (
        SELECT
            ...
            u.username,
            u.avatar_url,
            SUM(GREATEST(pt.amount, 0)) as total_points,
            ROW_NUMBER() OVER (ORDER BY SUM(GREATEST(pt.amount, 0)) DESC) as user_rank
        ...
        HAVING SUM(GREATEST(pt.amount, 0)) > 0
...
-- Create improved get_user_rank function
...
BEGIN
    ...
    -- Get user's total positive points for the period
    SELECT COALESCE(SUM(GREATEST(pt.amount, 0)), 0)
    INTO user_points
    FROM points_transactions pt
    ...
    FROM (
        ...
        HAVING SUM(GREATEST(pt2.amount, 0)) > user_points
    ) ranked_users;
